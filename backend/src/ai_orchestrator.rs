...
impl AIOrchestrator {
    ...
    async fn handle_trade(&self, params: HashMap<String, String>) -> Result<String, String> {
        ...
        match action {
            "execute" => {
                ...
                // Use default fee (5000 lamports) - fee optimizer would be used in TradingEngine path
                let tx_result = solana_client.execute_trade(symbol, size, is_buy, price, None).await?;
                drop(solana_client);
    ...
    async fn handle_ai_analysis(&self, params: HashMap<String, String>) -> Result<String, String> {
        if let Some(client) = &self.deepseek_client {
            ...
            let portfolio_value = engine.current_balance + engine.get_total_value(&HashMap::new());
            let action = params.get("action").unwrap_or(&"BUY".to_string()).clone();
            drop(engine);
            
            // Get price and volume history from market state
            let engine = self.trading_engine.lock().await;
            let price_history: Vec<f64> = if let Some(market_data) = engine.market_state.get(&symbol) {
                market_data.iter().map(|d| d.price).collect()
            } else {
                vec![price]
            };
            let volume_history: Vec<f64> = if let Some(market_data) = engine.market_state.get(&symbol) {
                market_data.iter().map(|d| d.volume).collect()
            } else {
                vec![1000.0]
            };
            drop(engine);
            
            // Execute analyze_trade and convert errors to String immediately to avoid Send issues
            let decision_result = {
                let client_guard = client.lock().await;
                client_guard.analyze_trade(
                    &symbol,
                    price,
                    &price_history,
                    &volume_history,
            ...
            match decision_result {
                Ok(decision) => {
                    // Also assess risk using assess_risk method
                    let risk_assessment = {
                        let client_guard = client.lock().await;
                        client_guard.assess_risk(
                            &symbol,
                            &action,
                            price,
                            size,
                            portfolio_value,
                        ).await.unwrap_or_else(|_| "Unable to assess risk".to_string())
                    };
                    
            Ok(format!(
                        "AI ANALYSIS: {} | Action: {} | Confidence: {:.2} | Risk: {} | Decision: {}",
                        symbol,
                        decision.action,
                        decision.confidence,
                        risk_assessment,
                ...
                Err(e) => {
                    // Error is already a String from map_err above
                    Err(format!("DeepSeek AI analysis failed: {}", e))
                }
            }
        } else {
    ...
    async fn handle_oracle(&self, params: HashMap<String, String>) -> Result<String, String> {
        let symbol = params.get("symbol").ok_or("Missing symbol")?;
        let _rpc_url = std::env::var("SOLANA_RPC_URL")
            .unwrap_or_else(|_| "https://api.devnet.solana.com".to_string());
        
        // OracleAggregator is initialized and ready
        // Note: This would need a SwitchboardClient to be created first
        // For now, this is just a placeholder
        // let switchboard_client = Arc::new(SwitchboardClient::new(rpc_url.clone(), true));
        // let _aggregator = crate::switchboard_oracle::OracleAggregator::new(switchboard_client);
