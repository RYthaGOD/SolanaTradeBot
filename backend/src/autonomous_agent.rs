...
impl AutonomousAgent {
    ...
    async fn fetch_oracle_data(&self) -> Result<HashMap<String, OracleFeed>, Box<dyn std::error::Error>> {
        ...
        let feeds = self.oracle_client.fetch_multiple_feeds(&symbols).await
            .map_err(|e| Box::new(std::io::Error::new(std::io::ErrorKind::Other, format!("Oracle error: {}", e))) as Box<dyn std::error::Error>)?;
    ...
    async fn fetch_dex_opportunities(&self) -> Result<Vec<TradingOpportunity>, Box<dyn std::error::Error>> {
        // Gracefully handle API failures - return empty list instead of error
        let opportunities = match self.dex_client.get_top_opportunities(5).await {
            Ok(opps) => {
                if opps.is_empty() {
                    log::debug!("No DEX opportunities found (Mobula API may be unavailable)");
                }
                opps
            }
            Err(e) => {
                log::debug!("Mobula API unavailable: {}. Continuing without DEX opportunities.", e);
                Vec::new() // Return empty instead of error
            }
        };
    ...
    async fn execute_trade(&self, decision: &AgentDecision) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Use trading_engine.execute_trade to handle the actual trade execution
        // Note: trading_enabled check is handled by the trading engine itself
        let success = engine.execute_trade(&signal, None, None).await;
        
        if !success {
            log::warn!("⚠️ Trade execution failed");
            return Ok(());
        }
