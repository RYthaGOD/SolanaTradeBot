...
use crate::pumpfun::{MemeTradeSignal, PumpFunClient};
use crate::risk_management::RiskManager;
use crate::switchboard_oracle::{OracleFeed, SwitchboardClient};
use crate::trading_engine::{TradeAction, TradingEngine, TradingSignal};
...
impl AutonomousAgent {
    ...
    pub async fn analyze_and_trade(&self) -> Result<AgentDecision, Box<dyn std::error::Error>> {
        ...
        let decision = self
            .make_composite_decision(oracle_feeds, dex_opportunities, meme_signals)
            .await?;
    ...
    async fn fetch_oracle_data(
        &self,
    ) -> Result<HashMap<String, OracleFeed>, Box<dyn std::error::Error>> {
        let symbols = vec![
            "SOL/USD".to_string(),
            "BTC/USD".to_string(),
            "ETH/USD".to_string(),
    ...
    async fn fetch_dex_opportunities(
        &self,
    ) -> Result<Vec<TradingOpportunity>, Box<dyn std::error::Error>> {
    ...
    async fn make_composite_decision(
        ...
        if !meme_signals.is_empty() {
            ...
            if top_meme.confidence > 0.7 {
                ...
                reasoning.push(format!(
                    "Strong meme signal: {} confidence {:.2}",
    ...
    async fn execute_trade(
        ...
        let position_size =
            self.calculate_position_size(decision.confidence, price, engine.current_balance);
    ...
    pub async fn get_stats(&self) -> HashMap<String, String> {
        ...
        stats.insert(
            ...
            format!("{:.2}", self.min_confidence),
        );
        stats.insert(
            "check_interval".to_string(),
            format!("{} seconds", self.check_interval_secs),
        );
        stats.insert(
            "total_trades".to_string(),
            engine.trade_history.len().to_string(),
        );
        stats.insert(
            "current_balance".to_string(),
            format!("${:.2}", engine.current_balance),
        );
        stats.insert(
            "active_positions".to_string(),
            engine.portfolio.len().to_string(),
        );
