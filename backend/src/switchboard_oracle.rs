...
impl SwitchboardClient {
    ...
    pub fn new_simulated() -> Self {
        Self::new("https://api.mainnet-beta.solana.com".to_string(), false)
    }
    ...
    async fn fetch_real_oracle_price(
        &self,
        symbol: &str,
        feed_address: &str,
    ) -> Result<OracleFeed, Box<dyn Error>> {
        log::info!(
            "Fetching real Switchboard feed for {} at {}",
            symbol,
            feed_address
        );

        let pubkey =
            Pubkey::from_str(feed_address).map_err(|e| format!("Invalid feed address: {}", e))?;
    ...
    async fn fetch_simulated_price(
        &self,
        symbol: &str,
        feed_address: &str,
    ) -> Result<OracleFeed, Box<dyn Error>> {
        log::debug!(
            "Using simulated Switchboard feed for {} at {}",
            symbol,
            feed_address
        );
    ...
    pub async fn fetch_multiple_feeds(
        &self,
        symbols: &[String],
    ) -> Result<Vec<OracleFeed>, Box<dyn Error>> {
...
impl OracleAggregator {
    ...
    pub async fn get_price_with_confidence(
        &self,
        symbol: &str,
...
mod tests {
    ...
    async fn test_production_client_creation() {
        let client =
            SwitchboardClient::new_production("https://api.mainnet-beta.solana.com".to_string());
        assert!(client.use_real_oracle);
        assert!(!client.feed_addresses.is_empty());
    }
