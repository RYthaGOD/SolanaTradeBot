...
impl HistoricalDataset {
    ...
    fn calculate_ema(&self, period: usize) -> Option<f64> {
        ...
        let prices: Vec<f64> = self.data.iter()
            .rev()
            .take(period)
            .map(|p| p.close)
            .collect();
    ...
    pub fn calculate_volatility(&self, period: usize) -> f64 {
        ...
        let returns: Vec<f64> = self.data.iter()
            .rev()
            .take(period + 1)
            .collect::<Vec<_>>()
            .windows(2)
    ...
    pub fn calculate_rsi(&self, period: usize) -> Option<f64> {
        ...
        let recent_data: Vec<_> = self.data.iter()
            .rev()
            .take(period + 1)
            .collect();
    ...
    pub fn generate_features(&self) -> HistoricalFeatures {
        ...
        let trend_strength = if let (Some(ema_10), Some(ema_50)) = (moving_averages.ema_10, moving_averages.ema_50) {
