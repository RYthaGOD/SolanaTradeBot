...
pub fn create_routes(
    ...
    let health = warp::path("health")
        .and(warp::get())
        .map(|| warp::reply::json(&ApiResponse::new("OK", "System is healthy")))
        .boxed();
                ...
                async move {
                    ...
                    match orchestrator.decide_action(orchestrator_req).await {
                        Ok(decision) => {
                            ...
                            match orchestrator
                                .execute_function(&decision.recommended_function, params)
                                .await
                            {
                                Ok(result) => {
                                    ...
                                    response
                                        .insert("priority", format!("{:.2}", decision.priority));
    ...
    let execute_function = {
        ...
        warp::path!("execute" / String)
            ...
            .and(warp::body::json())
            .and_then(
                move |function_name: String, params: HashMap<String, String>| {
                    let orchestrator = orchestrator.clone();
                    async move {
    ...
    let list_functions = {
        ...
        warp::path("functions")
            ...
            .map(move || {
                ...
                warp::reply::json(&ApiResponse::new(functions, "Available functions"))
            })
            .boxed()
    };
