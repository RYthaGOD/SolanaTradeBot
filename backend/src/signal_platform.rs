...
impl SignalMarketplace {
    ...
    async fn generate_oracle_signals(
        &self,
        provider_id: &str,
    ) -> Result<Vec<TradingSignalData>, String> {
        let symbols = vec![
    ...
    async fn generate_dex_signals(
        &self,
        provider_id: &str,
    ) -> Result<Vec<TradingSignalData>, String> {
        let opportunities = self
            .dex_client
            .get_top_opportunities(5)
            .await
    ...
    async fn generate_meme_signals(
        &self,
        provider_id: &str,
    ) -> Result<Vec<TradingSignalData>, String> {
        let meme_signals = self
            .pumpfun_client
            .get_top_opportunities(5)
            .await
    ...
    pub async fn get_signals_by_symbol(&self, symbol: &str) -> Vec<TradingSignalData> {
        ...
        signals
            .values()
            .filter(|s| {
                s.symbol == symbol && matches!(s.status, SignalStatus::Active) && s.expiry > now
            })
            .cloned()
            .collect()
    }
    ...
    pub async fn purchase_signal(
        &self,
        user_id: &str,
        signal_id: &str,
        payment: f64,
    ) -> Result<X402Message, String> {
        let mut signals = self.signals.lock().await;
        let mut providers = self.providers.lock().await;
        let mut subscriptions = self.subscriptions.lock().await;

        let signal = signals.get_mut(signal_id).ok_or("Signal not found")?;
    ...
    pub async fn process_x402_message(
        &self,
        message: X402Message,
    ) -> Result<Option<X402Message>, String> {
        match message.message_type {
            ...
            X402MessageType::SignalRequest => {
                if let X402Payload::Request { symbols, max_price } = message.payload {
                    ...
                    log::info!(
                        "🔍 Found {} matching signals for request",
            ...
            X402MessageType::SignalPurchase => {
                if let X402Payload::Purchase { signal_id, payment } = message.payload {
                    let confirmation = self
                        .purchase_signal(&message.sender_id, &signal_id, payment)
                        .await?;
    ...
    pub async fn cleanup_expired_signals(&self) {
        ...
        let expired: Vec<String> = signals
            .iter()
...
mod tests {
    ...
    async fn test_provider_registration() {
        ...
        let result = marketplace
            .register_provider("provider1".to_string(), "Test Provider".to_string())
            .await;
        assert!(result.is_ok());
    }
